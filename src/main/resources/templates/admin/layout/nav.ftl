    ...
    <div class="card">
      <div class="card-header p-0 pl-3" id="${navBar.id}">
        <button class="btn btn-link" type="button" data-toggle="collapse"
                data-target="#collapse-${navBar.id}" aria-expanded="${(navBar.id == breadcrumbs[0].id)?c}"
                aria-controls="collapse-${navBar.id}">
         ${navBar.name}
        </button>
      </div>
      <div id="collapse-${navBar.id}" class="collapse <#if navBar.id == breadcrumbs[0].id> show</#if>"
        ...
        <div class="card-body p-0">
          <div class="list-group list-group-flush pl-3">
            <#list navBar.children as child>
              ...
              list-group-item-action <#if breadcrumbs?size gte 2 && breadcrumbs[1].id == child
